...
set ejcmd (echo "lsof +D ./ | awk '{print $2}' | tail -n +2 | xargs -r kill -9")
alias ej_="echo '$ejcmd' | xclip -sel clip"
...
function chooseTheme
    ...
    if test "$checkOS" = Linux
        sed -i "670s/.*/ $choosen/" ~/.config/fish/config.fish && source ~/.config/fish/config.fish
    else
        gsed -i "670s/.*/ $choosen/" ~/.config/fish/config.fish && source ~/.config/fish/config.fish
    end
end
